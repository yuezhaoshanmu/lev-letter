  ...
  return (
    <main className="unlock-ritual" aria-live="polite">
      ...
      <div className="unlock-ritual-copy">
        ...
        <motion.p className="unlock-ritual-line" initial={reduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.1, delay: reduceMotion ? 0 : .85 }}>在翻开这本书之前。<br />我想先送你一首歌。</motion.p>
        <motion.p className="unlock-ritual-line unlock-ritual-line-soft" initial={reduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.1, delay: reduceMotion ? 0 : 1.7 }}>不是为了制造什么特别的气氛。<br />只是觉得。<br />如果这封信有一个声音，<br />我希望它是这一首。</motion.p>
        {showButton ? <motion.div className="music-choice-actions" initial={reduceMotion ? false : { opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .9 }}>
          ...
          <button className="music-choice-secondary" type="button" onClick={onSkipSound} disabled={busy}>先不用，我继续看看</button>
        </motion.div> : null}
      </div>
    </main>
  );
