...
export async function POST(request: Request) {
  ...
  const result = await createSupabaseAdmin().from("visitor_logs").insert({ id, session_id: sessionId, ip_address: ip, user_agent: ua, device: device(ua), last_page: "/" }).select("id").single();
