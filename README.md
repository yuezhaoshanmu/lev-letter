...
4. 进入 **Table Editor**，确认出现 `confession_responses` 表，包含 `choice` 和 `submitted_at`。
...
也可以在 Supabase 中直接进入 **Table Editor → confession_responses** 检查 `choice` 与 `submitted_at`。有效选择值为 `yes`、`no`、`thinking`。
